...
fn run() -> Result<(), Error> {
    ...
    let contact_sensor = cluster_contact_sensor::ContactSensorCluster::new(Dataver::new_rand(matter.rand()));
    ...
    let dm_handler = HandlerCompat(dm_handler(&matter, &contact_sensor));
    ...
    let mut device = pin!(async {
        loop {
            ...
            contact_sensor.set(!contact_sensor.get());
            subscriptions.notify_changed();

            info!("Contact toggled to {}", contact_sensor.get());
        }
    });
